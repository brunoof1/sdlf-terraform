  ...
  def main(sysArgs: Array[String]) {
    ...
    for (tabName <- tabNames) {
      ...
      verificationDataFrame.foreach{
        resultDF => writeDStoS3(resultDF, args("targetBucketName"), "constraints-verification-results", team, dataset, tabName, getYear, getMonth, getDay, getTimestamp)
      }
      if (!(analysisConstraintFullDF.isEmpty)) {
        analysisDataFrame.foreach{
          resultDF => writeDStoS3(resultDF, args("targetBucketName"), "constraints-analysis-results", team, dataset, tabName, getYear, getMonth, getDay, getTimestamp)
        }
      }
    }
